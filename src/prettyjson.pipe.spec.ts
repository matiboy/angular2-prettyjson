...
describe('Generated markup', () => {
  it('should include spans with correct classes', (done: Function) => {
    ...
    htmltree(outcome, (err: any, htmlDoc: any) => {
      ...
      [0, 2, 4, 6, 9, 10, 11].forEach((index: number) =>
        expect(spans[index].attributes.class).to.equal('key')
      );
      [1, 3, 7].forEach((index: number) =>
        expect(spans[index].attributes.class).to.equal('number')
      );
      [5, 8].forEach((index: number) =>
        expect(spans[index].attributes.class).to.equal('string')
      );
